...
public class TeclatHandler implements OnKeyListener {
	...
	//Guardem de cada tecla si està premuda o no. Cada posició correspon //al codi de la tecla
	boolean[] teclesPremudes = new boolean[128];
	//Fons d’instàncies d’events de teclat
	Fons<EventTeclat> fonsEvents;
	//Aquí guardarem els events que encara no han estat processats 
	List<EventTeclat> bufferEvents = new ArrayList<EventTeclat>();
	//Aquí guardarem els events que retornem amb getEventsTeclat
	...
	@Override
	//Implementació del’OnKeyListener.onKey
	//Creem un event de teclat vàlid i l’afegim al buffer d’events de teclat
		...
		synchronized (this)
		{
			...
			//Tecla premuda
				if (event.getAction() == android.view.KeyEvent.ACTION_DOWN) {
					...
					if(keyCode > 0 && keyCode < 127) teclesPremudes[keyCode] = true;
				}
			//Tecla aixecada
				if (event.getAction() == android.view.KeyEvent.ACTION_UP) {
					...
					//Actualitzem l’array de tecles premudes
					if(keyCode > 0 && keyCode < 127) teclesPremudes[keyCode] = false;
				}
	...
	//Es cridarà des d’un fil diferent 
		synchronized (this)
		{
