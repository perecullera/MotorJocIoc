...
public class MotorAudio implements Audio {
	...
	public Musica nouMusica(String nomFitxer) {
		try {
			...
			//Creem el nou recurs de música
			return new MotorMusica(assetDescriptor);
			} catch (IOException e) {
			throw new RuntimeException("No pot carregar fitxer de música:" + nomFitxer);
			}
	}
	...
	public So nouSo(String nomFitxer) {
		try {
			...
			throw new RuntimeException("No pot carregar fitxer de so:" + nomFitxer);
		}
	}
