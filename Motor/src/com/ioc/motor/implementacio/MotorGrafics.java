...
public class MotorGrafics implements Grafics {
	...
	//framebuffer que contindrà el contingut de la pantalla abans d’enviar−la 
	Bitmap frameBuffer;
	//Per poder dibuixar
	Canvas canvas;
	//Les guardem aquí per no haver de crear noves instàncies cada vegada 
	...
	public MotorGrafics(AssetManager assets, Bitmap frameBuffer) {
		...
		//Creem el canvas perquè dibuixi en el framebuffer (Bitmap) 
	...
	@Override
	//Dibuixa un píxel per pantalla
	...
	@Override
	//Dibuixa una línia per pantalla
	public void dibuixaLinia(int origenX, int origenY, int destiX, int destiY,
			int color) {
		paint.setColor(color);
	...
	public Pixmap nouPixmap(String nomFitxer, PixmapFormat format) {
		Config config = null;
		//Transformació entre els formats propis de Pixmap i els d’Android 
		...
		try
		{
		  ...
		  if (bitmap == null)
			  throw new RuntimeException("No es pot carregar gràfic ’"+ nomFitxer + "’");
		}
		catch (IOException e) {
			throw new RuntimeException("No es pot carregar gràfic ’"+ nomFitxer + "’");
		}
		...
		//Obtenim el format després de la conversió que fa el propi Android 
	...
	@Override
	//Dibuixa un Pixmap per pantalla. Permet dibuixar únicament una regió del Pixmap original
	...
	public void dibuixaPixmap(Pixmap pixmap, int x, int y, int origenX,
		...
		//Creem el rectangle de destí on es dibuixarà
	    rectDesti.left = x;
	    rectDesti.top = y;
	...
	@Override
	//Retorna l’alçada del display (en aquest cas el nostre Bitmap que fa de framebuffer)
	public int getAlt() {
